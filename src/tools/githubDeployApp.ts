...
import { z } from 'zod';
import { type FileEntry, GitHubClient, liveURL, repoForTarget } from '../github/github.js';
...
Commit message is "deploy: <app_name>" for new apps, "update: <app_name>" when replacing.`,
    ...
    async ({ app_name, repo: repoTarget, files: filesJSON, author_name, author_email }) => {
      ...
      // Parse files
      if (!filesJSON) {
        return { content: [{ type: 'text', text: 'files is required' }], isError: true };
      }
      ...
      try {
        ...
        return {
          content: [{ type: 'text', text: `files must be a valid JSON array: ${msg}` }],
          isError: true,
        };
      }
      ...
      for (const f of entries) {
        if (!f.path) {
          return {
            content: [{ type: 'text', text: 'each file entry must have a non-empty path' }],
            isError: true,
          };
        }
      ...
      if (repoTarget === 'public') {
        const findings = scan(entries.map((e) => ({ path: e.path, content: e.content })));
        if (findings.length > 0) {
          ...
          return { content: [{ type: 'text', text: errorMessage(findings) }], isError: true };
        }
      }
      ...
      log('info', 'github-deploy-app: checking app existence', { app: app_name, repo: repoTarget });
      let exists: boolean;
      try {
        exists = await client.appExists(owner, repoName, app_name);
      } catch (e) {
        const msg = e instanceof Error ? e.message : String(e);
        log('error', 'github-deploy-app: existence check failed', { error: msg });
      ...
      lines.push('');
      lines.push(`**Commit:** [${result.commitSHA.slice(0, 8)}](${result.commitURL})`);
      lines.push('');
